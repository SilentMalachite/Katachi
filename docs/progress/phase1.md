...
## 2026-08-09 — PR #2 を main へマージ。Phase 1 クローズ

### 実施内容

`docs/phases.md` §1.5.1 を追記したうえで PR を作成し、`main` へマージした。
「1 Phase = 1 ブランチ = 1 PR」（`phases.md` §1）の完了。

| 項目 | 内容 |
|---|---|
| PR | [#2](https://github.com/SilentMalachite/Katachi/pull/2) |
| 差分 | 39 ファイル（追加 30 / 変更 9）、+4723 行 / -18 行、13 コミット |
| マージ方式 | **merge commit**（squash しない） |
| マージコミット | `e3289ee` |

### 変更ファイル

**変更**: `docs/phases.md`（§1.5.1 の新設）。他はマージ操作のみでコード変更なし。

### `docs/phases.md` §1.5.1 の新設

CI のモジュール指定を指示書側にも記録した。**同じ罠を再び踏まないため。**

書いたのは表だけではなく、**症状の現れ方**である。

- 能力表は実行時生成のため、モジュール欠落は**ビルドエラーにならない。**
  「対応形式が減る」という形で静かに現れる
- ローカルは公式インストーラの全部入りのため、
  **ローカル green / CI のみ失敗**という現れ方をする

この 2 点を書かないと、次に同じことが起きたときに原因へ辿り着けない。

### 品質ゲートの実行結果

マージ前後で計 4 回、全ジョブ green を確認した。

| タイミング | run | 結果 |
|---|---|---|
| `phase1` への push（`88da6fc`） | 31289352650 | 4 ジョブ success |
| PR 作成による `pull_request` トリガ | 31289362405 | 4 ジョブ success |
| **マージ後の `main`（`e3289ee`）** | **31289572774** | **4 ジョブ success** |

マージ直前に `gh pr checks` で **8 / 8 pass、pending・fail は 0**、
`mergeStateStatus=CLEAN` を確認してからマージした。
**初回 CI が 4 ジョブとも落ちた実績があるため、pending のままマージしていない。**

マージコミット自体の CI も green であり、`main` が壊れていないことを確認済み。
...
| 項目 | 到達点 |
|---|---|
| テスト | 16 → **93**（Qt 6.8.3 の macOS / Windows とも 93 / 93） |
| 品質ゲート | 6 本すべて exit 0、警告ゼロ、clang-tidy 指摘 0 件 |
| 受け入れ基準（`phases.md` §4） | **10 項目すべて達成** |
| 新規 ADR | 0002〜0007 の **6 本** |
| 停止して判断を仰いだ矛盾 | **7 件**。すべて指示を得てから進めた |
| 抑制コメント（`NOLINT` 等） | **0 件** |

**Phase 1 はクローズする。**

### この Phase で学んだこと（次の Phase へ引き継ぐ）

1. **品質ゲートは実行前に `git add` でステージする。**
   `git ls-files` は追跡済みファイルしか列挙せず、新規ファイルが検査から漏れる（T2 で実際に起きた）。
2. **CI の成否は `gh run view --json conclusion` で判定する。**
   `gh run watch --exit-status` は run が failure でも exit 0 を返す（Phase 0 で確認）。
3. **ctest の結果は build の成否を確認してから読む。**
   ビルドが失敗しても古いテストバイナリが走り「全部 pass」に見える（T1 で実際に起きた）。
4. **リンタと指示書が衝突したら、コードを歪めずに停止して判断を仰ぐ。**
   `QChar` の並べ書きや `std::vector` の回避で検査を回避しない。
5. **サブエージェントの調査結果は鵜呑みにせず、自分のテストで再現させてから直す。**
   T5 で欠陥 2 件を再現させてから修正した。BMP が ICC を保持できないことも自分で実測した。

### 残課題 / 次にやること

1. **`phase1` ブランチの削除。** 未実施。
2. **Windows の実機起動確認。** Phase 0 から未了。CI では通っているが実機未確認。
3. **Phase 2（GUI）の計画提示と承認。** 未着手。着手時の宿題は次のとおり。
   - `docs/phases.md` §5.3（バッチ実行時のメモリ上限）
   - 衝突ポリシー（`Overwrite` / `Skip` / `Rename`、既定 `Skip`）の `src/io` 層での実装。
     **既定値を確認せずに `Overwrite` にしないこと**（ADR-0005）
   - ICO を読むと Qt が注入する内部キー `_q_icoOrigDepth` の扱い
   - `src/io/IoConcepts.hpp` の concept 3 種（`ByteSource` / `ByteSink` / `ProgressSink`）。
     **`ProgressSink::onProgress` に `noexcept` を要求しない**（`cpp-conventions.md` §2.4）
