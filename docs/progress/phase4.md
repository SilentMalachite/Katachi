...
## 2026-08-11 — PR #6 をマージ。**Phase 4 はクローズしない**（受け入れ基準 2 件が未達）

### 実施内容

`docs/phases.md` §1「1 Phase = 1 ブランチ = 1 PR」に従い PR #6 を作成し、
CI green を確認して `main` にマージした。

**Phase 3 と違い、「Phase 4 を完了した」とは書かない。** 受け入れ基準 5 件のうち
2 件が未達のままであり、それは利用者の環境と資格情報を要する項目だからである。

### PR とマージの結果

| 項目 | 値 |
|---|---|
| PR | [#6](https://github.com/SilentMalachite/Katachi/pull/6) `Phase 4: 配布（macOS の .dmg / Windows の zip・インストーラ / 第三者ライセンスの同梱）` |
| merge commit | **`00a0019`**（マージコミット方式。PR #2 / #3 / #5 と同じ） |
| 取り込み | **30 コミット / 54 ファイル / +5,879 行 / -19 行** |
| `src/` の変更 | **`src/app/CMakeLists.txt` の 1 ファイルのみ。C++ のコードは 1 行も変えていない** |
| `phase4` ブランチ | マージ後に削除 |

**マージ前に PR の checks 7 件すべてが SUCCESS、`mergeStateStatus` が `CLEAN` で
あることを確認した。** 実行中の状態ではマージしていない。

### マージ後の `main` の検証（ローカル macOS 26.6.1 / arm64、Qt 6.11.1、LLVM 22.1.8）

| # | 実行 | 結果 |
|---|---|---|
| 1 | `cmake --build --preset dev` | **警告 0** |
| 2 | `ctest --preset dev` | **172 / 172 pass** |
| 3 | `clang-format --dry-run --Werror` | **指摘なし** |
| 4 | `clang-tidy -p build/dev` | **指摘 0 件** |
| 5 | `ctest --preset asan` | **172 / 172 pass** |
| 6 | `ctest --preset release` | **192 / 192 pass** |
| 7 | `LICENSE` の md5 | **Phase 3 時点（`4e9678c`）と一致。無変更** |

### Phase 4 の到達点

`KATACHI_PACKAGE=ON`（`release` プリセット）で、両 OS の配布物を組み立てて
機械検査まで通せる。CI の `package` ジョブが毎 PR でこれを行う。

| 成果物 | 実測 |
|---|---|
| `Katachi.app` | 52 MB / universal（`x86_64 arm64`）/ `minos 13.0` |
| `Katachi-0.1.0.dmg` | 24 MB / **Developer ID 署名済み・未公証** |
| `Katachi-0.1.0-windows-x64.zip` | 18 MB |
| `Katachi-0.1.0-setup.exe` | 14 MB / **未署名** |

`macdeployqt` / `windeployqt` の素の出力から**使わないものを削り**、
**落とされるものを戻し**、**権利表示を機械的に生成**している。

### **未達のまま残すもの（「全部できた」とは書かない）**

| 項目 | 状態 |
|---|---|
| **macOS の公証** | **未実施。** 資格情報が未取得。手順は `docs/release.md` §3 |
| **クリーンな環境での起動確認**（受け入れ基準 5） | **未確認。** 機械的な代替（P8 / P9）は CI で走るが、実機で確かめていない |
| **日本語入力の確認** | **未確認。** `platforminputcontexts` を削った影響。`cocoa` が IME を担うという推論を実機で確かめていない |
| **GPU ドライバの無い Windows** | **未確認。** `opengl32sw.dll` を残す判断の根拠 |
| **Windows 実機での起動** | **未確認。** Phase 2 から継続 |
| **Windows での追加コーデック** | **未対応。** Phase 3 から継続 |
| **キーボードのみでの実操作** | **未確認。** Phase 2 から継続 |
| `docs/format-matrix.md` の生成先がツリー間で共有 | **未修正。** Phase 3 からの提案のまま |
...
| # | 内容 |
|---|---|
| 8 | 仮想キーボードを外して macOS の日本語入力に影響が無いか |
| 11 | `opengl32sw.dll` が GPU ドライバの無い環境で要ること |

### 残課題 / 次にやること

1. **公証**（`docs/release.md` §3）。資格情報を用意して提出 → staple → `stapler validate`
2. **T8: 実機確認**（同 §5）。上の未達表の 4 件
3. すべて済んだ時点で `docs/phases.md` §4 の基準 1 と 5 を `- [x]` にし、
   **そこで初めて Phase 4 のクローズを記録する**

---

## 2026-08-17 — 公証を任意にし、Windows 基準 5 を CI のみにした

### 実施内容

利用者の承認（案 A × 2）により、Phase 4 の受け入れ読みを更新した。**コードは変更していない。**

| # | 決定 | 内容 |
|---|---|---|
| D15 | macOS 公証 | **任意。** 基準 1 は Developer ID 署名まで。未公証時の受け手向け手順を文書化 |
| D14 | Windows 基準 5 | **CI（P9 / `package` ジョブ）で足りる。** 実機は必須としない |
| — | macOS 基準 5 | **実機のまま残す**（Qt 無し別アカウント） |

### 読んだ文書

- `CLAUDE.md`
- `docs/progress/phase4.md`（本ファイルの直前まで）
- `docs/phases.md` §4 Phase 4
- `docs/release.md`
- `docs/adr/0015-packaging.md`
- `README.md`
...
| 操作 | パス |
|---|---|
| 変更 | `docs/release.md`（§3.0 新設、公証を任意化、§5 を OS 別に分割） |
| 変更 | `docs/phases.md`（基準 1・5、D2/D5 の注記、D14/D15） |
| 変更 | `docs/adr/0015-packaging.md`（末尾追記のみ。既存本文は無改変） |
| 変更 | `docs/progress/phase4.md`（本追記） |
| 変更 | `README.md`（状態・未確認項目・配布物の注意） |
...
文書のみのため、ビルド / テスト / clang-* / ASan は実行していない。
**「通った」とは書かない。**

### 推測で埋めた箇所

なし。Gatekeeper の `Unnotarized Developer ID` 表示と署名済み・未公証の実測は
本ファイルの既存記録に基づく。
...
| 1 | **達成**（公証は任意） |
| 2〜4 | **達成**（従来どおり） |
| 5 | **部分達成**（Windows = CI で達成、macOS 実機は未確認） |

### 残課題 / 次にやること

1. **T8: macOS 実機確認**（`docs/release.md` §5.1）。下の「T8 実機確認（記入用）」を埋める
2. 全部 OK なら `docs/release.md` §5.3 のクローズ手順を実行する（基準 5 を `[x]` → Phase 4 クローズ追記）
3. 公証は任意のまま残す。行う場合は `docs/release.md` §3.1 以降
4. Windows 実機・GPU 無し環境・キーボードのみ操作などは、引き続き「未確認」として残す（必須ではない）

---

## T8 実機確認（記入用）— **まだ未実施。結果を書くまでクローズしない**

利用者が macOS のクリーン環境で確認したあと、この節を**追記で埋める**。
手順の本体は `docs/release.md` §5.1。クローズ作業の順序は同 §5.3。
...
| 日付 | YYYY-MM-DD |
| macOS 版 | （例: 15.x） |
| マシン | （例: Apple Silicon / Intel） |
| ユーザーアカウント | Qt 公式インストーラ・Homebrew の `qt` が**無い**ことを確認した別アカウント名 |
| 配布物 | （例: `Katachi-0.1.0.dmg` / 署名の有無・未公証） |

Qt が無いことの確認コマンドと出力（貼る）:

```text
# ls ~/Qt
# brew list 2>/dev/null | grep -i '^qt'
（ここに出力を貼る）
```
...
| # | 項目 | 結果（OK / NG） | メモ |
|---|---|---|---|
| 1 | `.dmg` を開き Applications へコピーできた |  |  |
| 2 | 未公証経路: 右クリック →「開く」で起動できた（または隔離属性除去後に起動） |  |  |
| 3 | 日本語入力ができる |  |  |
| 4 | 画像を D&D して変換できた |  |  |
| 5 | 対応形式一覧に svg / svgz がある |  |  |

**すべて OK になるまで** `docs/phases.md` の基準 5 を `- [x]` にしない。
すべて OK のあと、`docs/release.md` §5.3 の 1〜5 を実行する。

### Phase 4 クローズ（記入用）— §5.3 実行時に追記

```markdown
## YYYY-MM-DD — Phase 4 をクローズする

### 実施内容
T8 実機確認がすべて OK。`docs/phases.md` 基準 5 を [x] にした。README の状態を更新した。

### 変更ファイル
| 操作 | パス |
|---|---|
| 変更 | docs/phases.md |
| 変更 | docs/progress/phase4.md |
| 変更 | README.md |
...
### 品質ゲートの実行結果
（実機確認のみなら「文書更新のみ。ゲート未実行」と書く）

### 推測で埋めた箇所
なし。

### 残課題 / 次にやること
（公証・Windows 実機など、必須でない未確認を列挙）
```

---

## 2026-08-17 — Phase 4 クローズ手順を文書化した（実機・クローズ自体は未実施）

### 実施内容

利用者選択 B: 実機確認は利用者が行い、**文書上のクローズ手順と記入テンプレートだけ先に用意する。**
基準 5 は `[ ]` のまま。Phase 4 はクローズしない。

### 変更ファイル

| 操作 | パス |
|---|---|
| 変更 | `docs/release.md`（§5.3 クローズ手順） |
| 変更 | `docs/phases.md`（クローズ手順への参照） |
| 変更 | `docs/progress/phase4.md`（本追記 + T8 記入用） |
...
1. 利用者が T8 記入用を埋める
2. すべて OK なら `docs/release.md` §5.3 を実行（またはエージェントに依頼）
