...
## 2026-08-09 — T8: 受け入れ基準を検証した。**3 項目すべて達成**

### 実施内容

`docs/phases.md` §4 Phase 3 の 3 項目を、**機械で確かめられる根拠**に基づいて検証した。
`docs/phases.md` のチェックボックスを更新し、根拠の表を添えた。

### 検証（推測ではなくコマンドの出力に基づく）

**基準 1: 追加コーデックの導入前に `docs/licenses.md` が更新されている**

`git log --reverse main..HEAD` の順序で確認した。

```
f916862 docs: T2 の調査結果（追加コーデックのライセンスと導入方式）を記録   ← licenses.md 更新
52e92ca build: 追加コーデックのオプショナル依存の枠組みを入れる（既定 OFF）  ← 導入
```

**licenses.md の更新が先行している。** §5.1 に 6 対象（libavif / libjxl / LibRaw /
kimageformats の psd・raw / 同 avif・jxl / ECM）の版・ライセンス・出典 URL・
GPLv3 との両立可否を記録した。§5.2 には x265 についての訂正がある。**達成。**

**基準 2: 追加コーデックが無い環境でもビルド・起動する（オプショナル依存）**

```
$ otool -L build/dev-codecs/src/app/katachi | grep -icE 'avif|jxl|libraw|kimg'
0
```

**ON でビルドしたアプリですら追加コーデックにリンクしていない。**
依存は Qt（Widgets / Concurrent / Gui / Core）とシステムフレームワークのみである。
プラグインは実行時に読み込まれる共有ライブラリなので、無ければ無いまま動く。

既定は OFF であり、**日常の 6 ゲートと CI の 4 ジョブはすべて「追加コーデックが無い環境」**を
検証している（両 OS でビルド・テスト green）。**達成。**

**基準 3: 能力表が追加コーデックを自動的に反映する（コード変更不要）**

```
$ git diff main...HEAD --name-only -- src/ | wc -l
0
```

**ブランチ全体で `src/` の変更は 0 ファイル。** それでも:

| | macOS / Qt 6.8.3（CI） | macOS / Qt 6.11.1（ローカル） |
|---|---|---|
| OFF | 21 形式 | 21 形式 |
| ON | **57 形式** | **57 形式** |

増えた 36 形式の顔ぶれも両環境で一致した。加えて、テスト専用プラグインによる
機械検査 6 本（T4）が CI で走り続ける。**達成。**

### `docs/phases.md` の更新について（申告）

Phase 3 の 3 項目を `- [x]` にし、根拠の表を添えた。
**Phase 0〜2 のチェックボックスは未更新のまま残した。** Phase 3 の作業範囲を超えるためである
（`docs/agent-protocol.md` §6「ついでにを禁止」）。まとめて直すなら別の変更として行う。

### 変更ファイル

- 変更: `docs/phases.md`（§4 Phase 3 のチェックボックスと根拠の表）、`docs/progress/phase3.md`（本エントリ）
...
| 実行 | 結果 |
|---|---|
| ローカル 6 ゲート（OFF 構成） | **172 / 172 pass**、警告 0、clang-format / clang-tidy 指摘なし、asan も 172 / 172 |
| ローカル ON 構成（`dev-codecs`） | ビルド exit 0 / **172 / 172 pass** |
| CI run 31318145842（5 ジョブ） | **すべて success** |
...
### Phase 3 の到達点と、達成していないこと

**受け入れ基準 3 項目はすべて達成した。** ただし「全部できた」ではない。
次は Phase 3 の範囲外として明示的に残す。

| 項目 | 状態 |
|---|---|
| **Windows での追加コーデック** | **未対応。** ON ジョブは macOS のみ。vcpkg でのライブラリ調達が要る（T3 で申告して承認済み） |
| **HEIF** | **対象外。** macOS は `qmacheif` で読み書き可、Windows は使えないまま（ADR-0013） |
| **EXIF 全体の保持（`PreserveAll`）** | **見送り。** ADR-0014 に条件付きで将来へ持ち越した |
| **多フレーム GIF** | **非スコープのまま**（`docs/spec-core.md` §8） |
| `docs/format-matrix.md` の生成先がツリー間で共有 | **未修正。** 提案のまま |
| Phase 2 の未達 2 件（キーボードのみの実操作 / Windows 実機起動） | **Phase 3 とは独立に未達のまま** |
