...
## 2026-08-09 — T10 完了（受け入れ基準の検証）。**実機での操作確認だけが残る**

### 実施内容

`docs/phases.md` §4 Phase 2 の受け入れ基準を、機械で確かめられる範囲まで固定した。
1000 件のバッチ、UI の応答性、`docs/spec-core.md` §7 の機械検査 3 件、メモリ予算のピーク。
...
| ファイル | 内容 |
|---|---|
| `tests/ui/scan_ui_requirements.cmake` | §7 の機械検査（UI1 アニメーション / UI2 独自テーマ / UI3 複数ウィンドウ） |
| `tests/ui/fixtures/violations/ui1..ui3/` | 各検査の違反フィクスチャ |
| `cmake/UiRequirements.cmake` | 上記を ctest に登録（検査 3 + 検出確認 3） |
| `tests/io/large_batch_test.cpp` | 1000 件のバッチ |
| `tests/app/batch_responsiveness_test.cpp` | `MainWindow` を通した応答性 |

**変更**

| ファイル | 内容 |
|---|---|
| `CMakeLists.txt` | `include(cmake/UiRequirements.cmake)` |
| `src/io/JobRunnerBridge.hpp/.cpp` | `peakBudgetUnits()` / `budgetTotalUnits()` を追加（検証のため公開） |
| `tests/CMakeLists.txt` | 新規テストの登録 |

**削除**: なし

### 設計上の判断

**UI 検査は不変条件スキャナと別のファイルに置いた**（ADR-0010 の決定どおり）。
`tests/ui/scan_ui_requirements.cmake` と `cmake/UiRequirements.cmake`。
由来が違うものを混ぜると、どちらの根拠で落ちているのか分からなくなる。

**コメントは検査対象から外す。** 「アニメーションを使わない」と説明するコメントまで
違反にしない（INV3A / INV4 と同じ扱い）。

### 承認された計画からの変更（申告）

**1. 応答性の判定を「発火回数」から「最大の間隔」に変えた。**

計画の期待値は「main thread の `QTimer(10ms)` が **20 回以上**発火する」だったが、
**実測で 200 件のバッチが約 40ms で終わり、4 回しか発火しなかった。**

発火回数はバッチの所要時間に比例するため、**閾値を置くと「速い機械では落ちる」テスト**になる。
確かめたい性質は「main thread が塞がっていないこと」であり、それは
**タイマ発火の最大の間隔**で測るのが正しい。`maxGap < 500ms` を主たる判定にした。
1000 件のテストでも同じ形に揃えた。

**2. 1000 件を流すのは io 層のテストだけにした。**
計画では app 層にも同規模を置く形だったが、**同じ規模を 2 度流すと CI の時間が二重にかかる。**
`MainWindow` を通す経路の確認は 200 件で行い、負荷の確認は 1000 件の io テストが担う。

**3. UI 検査に違反フィクスチャを付けた**（計画に無い追加）。
不変条件スキャナと同じく、**空振りしていないことを機械で確かめるため。**

**4. `peakBudgetUnits()` / `budgetTotalUnits()` を `JobRunnerBridge` に足した**（計画に無い追加）。
予算のピークを外から見る手段が無いと、受け入れ基準を検証できない。
T6 の `lastWorkerThread()` と同じ扱い。

### 追加・変更したテスト（8 本追加。158 → 166）

| テスト | 期待値 | 結果 |
|---|---|---|
| `a batch of 1000 files completes without blocking the main thread` | **1000 件すべて成功**、出力も 1000 件。タイマの**最大間隔 < 500ms**。進捗シグナル **< 100 回**。予算のピークが **0 < peak <= total** | pass |
| `the window stays responsive during a batch` | `MainWindow` 経由で 200 件が全件成功。**最大間隔 < 500ms**。完了後も行が残り、ステータス行に文言が出る | pass |
| `ui.ui1` / `ui.ui1.detects_violation` | `src/` に `QPropertyAnimation` 等が無い / 違反フィクスチャで落ちる | pass |
| `ui.ui2` / `ui.ui2.detects_violation` | `src/` に `setStyleSheet` / `setPalette` 等が無い / 落ちる | pass |
| `ui.ui3` / `ui.ui3.detects_violation` | `src/` に `QDockWidget` 等が無い / 落ちる | pass |

**1000 件のテストは ASan / UBSan 版でも通る**（並列実行下でのメモリエラーが無いこと）。

### UI 検査が空振りでないことの確認

違反フィクスチャに加えて、**実ファイルで検出できることを確かめた。**

| 確認 | 結果 |
|---|---|
| `src/app/MainWindow.cpp` に `setStyleSheet` を一時的に追加 | **UI2 が検出**（exit 1）。UI1 / UI3 は反応せず（誤検出なし） |
| 一時的な追加を戻す | 3 検査とも ok。`git diff` が空であることも確認 |

### `docs/phases.md` §4 Phase 2 の受け入れ基準

| 項目 | 状態 | 根拠 |
|---|---|---|
| ドラッグ&ドロップでファイル / フォルダを追加できる | **部分的に達成。実機未確認** | フォルダの再帰・重複排除・拡張子判定はテスト済み（T9）。**`dropEvent` から `addSources` への実配線は自動テストで確かめられない**（合成イベントが配送されない。T9 に実測を記録）。**実機確認が要る** |
| 変換中にキャンセルでき、キャンセル後にアプリが正常状態に戻る | **達成** | T6: キャンセル後に `finished` が届き `isRunning()` が false、**再投入して全件成功** |
| 1000 ファイルのバッチで UI が固まらない | **達成** | T10: 1000 件全件成功、タイマの最大間隔 < 500ms |
| `spec-core.md` §7 の UI 非機能要件を全て満たす | **6 / 7 達成。1 項目が実機未確認** | 下表 |
| ワーカースレッドから `QWidget` に触れていないことを確認済み | **達成** | `katachi_io` が `Qt6::Widgets` をリンクしない（リンク行の実測）+ 不変条件 INV7 + 実行時テスト（変換が main と別スレッド） |
| 失敗したジョブが結果一覧に理由付きで残る | **達成** | T7 / T9: 失敗行が消えず理由列が埋まる。スキップは失敗と別表示 |

**`spec-core.md` §7 の 7 項目**

| 要件 | 状態 | 根拠 |
|---|---|---|
| アニメーションを使わない | **達成** | `ui.ui1`（機械検査） |
| 進捗は 200ms 以下の間隔で更新しない | **達成** | T6: タイマ間隔 200、1000 件で進捗シグナル 100 回未満 |
| ダークモードはシステム設定に追随 | **達成** | `ui.ui2`（独自テーマを書いていない） |
| 全機能がキーボードのみで操作可能 | **未確認** | タブ順の連結と到達可能性はテスト済み（T9）。**実際にキーボードだけで操作できるかは実機確認が要る** |
| 自動スクロール禁止 | **達成** | T9: 100 行追加してもスクロール位置が変わらない |
| モーダルは破壊的操作の確認のみ | **達成** | T9: 失敗してもモーダルが開かない。モーダルは出力先選択と上書き確認の 2 用途のみ |
| ウィンドウは単一 | **達成** | `ui.ui3`（機械検査） |
...
**なし。** 応答性の判定を変えたのは、実測（200 件が 40ms で完了し 4 回しか発火しない）に基づく。

### 残課題 / 次にやること

1. **CI で確認する。** push して 4 ジョブ green を確かめる。
   **1000 件のテストは Windows で時間がかかる可能性がある**（ファイル生成が macOS より遅い）。
   落ちたり極端に遅かったりしたら報告する。
2. **利用者にお願いする実機確認。** 下記が残っており、**これが済むまで
   Phase 2 の受け入れ基準は「全項目達成」とは書けない。**
   - ファイル / フォルダを実際に D&D して行が増えること
   - キーボードのみで開始・キャンセルできること
   - Windows での実機起動（Phase 0 から未了）
3. Phase 2 の PR 作成（`phases.md` §1「1 Phase = 1 ブランチ = 1 PR」）。
4. `.serena/` は未追跡のまま。

---

## 2026-08-09 — 利用者による実機確認（macOS / Apple Silicon）。**D&D と変換の実行が確認できた**

### 実施内容

T9 / T10 で「エージェントは GUI を実操作できないため利用者の確認をお願いする」と記録した
項目のうち、**2 項目について利用者から「確認できた」との報告を受けた。** その内容を記録する。

**この記録の出所は利用者の実機操作であり、エージェントは GUI を実操作していない。**
自動テストや CI の結果を実機操作の根拠にはしていない（`docs/phases.md` §4 Phase 0 と同じ扱い）。

### 確認できた項目

| 確認項目 | 状態 | 出所 |
|---|---|---|
| ファイル / フォルダを D&D して行が増える | **確認済み** | 利用者の実機操作（macOS / Apple Silicon） |
| 変換を実行し、結果が表示される | **確認済み** | 同上 |

**T9 で「合成ドロップイベントが配送されないため自動テストでは確かめられない」と実測付きで
記録した `dropEvent` → `addSources` の実配線が、これで確認できた。**
自動テストで担保していたのは `addSources` 以降（再帰・重複排除・能力表による判定）だけであり、
その手前の一区間が実機で埋まったことになる。

### 確認できていない項目（引き続き残る）

| 確認項目 | 状態 | 備考 |
|---|---|---|
| キーボードのみで開始・キャンセルできる | **未確認** | タブ順の連結と到達可能性は T9 で自動テスト済み。**マウス無しで実際に操作できるかは未確認** |
| 変換中のキャンセルと、その後の再実行（実機） | **未確認** | 自動テストは T6 で pass。実機操作としては未確認 |
| Windows での実機起動 | **未確認** | Phase 0 から。今回の確認は macOS / Apple Silicon のみ |

**利用者が確認したと答えなかった項目を「たぶん動く」とは書かない。**

### `docs/phases.md` §4 Phase 2 受け入れ基準の更新

| 項目 | 更新前（T10 時点） | 更新後 |
|---|---|---|
| ドラッグ&ドロップでファイル / フォルダを追加できる | 部分的に達成。実機未確認 | **達成**（利用者の実機確認） |
| `spec-core.md` §7 の UI 非機能要件を全て満たす | 6 / 7 達成。1 項目が実機未確認 | **6 / 7 達成のまま**（キーボード操作が未確認） |

他の 4 項目は T10 の記録から変更なし。

### 品質ゲートの実行結果

**今回はコードを変更していないため、ゲートは実行していない。**
直近の結果は T10 の記録（dev / asan とも 166 / 166 pass、警告 0）のとおり。

### 推測で埋めた箇所

**なし。** 利用者が確認したと答えた 2 項目だけを「確認済み」とし、
答えの無かったキャンセル操作とキーボード操作は未確認のまま残した。
D&D についても「行が増えた」ことだけを記録し、フォルダの再帰・重複排除・
非対応ファイルの除外が**実機で**確かめられたかは書いていない（自動テストでは pass）。

### 残課題 / 次にやること

1. **push して CI の 4 ジョブを確認する。** ローカルは `origin/phase2` より 3 コミット先行している。
   **1000 件のテストが Windows で遅い可能性がある**（T10 に記録）。
2. **キーボードのみでの開始・キャンセル**（利用者にお願いする）。
...
## 2026-08-09 — PR #3 をマージし Phase 2 をクローズ。**実機確認 2 点は未達のまま残す**

### 実施内容

`docs/phases.md` §1「1 Phase = 1 ブランチ = 1 PR」に従い PR #3 を作成し、
...
| 項目 | 値 |
|---|---|
| PR | [#3](https://github.com/SilentMalachite/Katachi/pull/3) `Phase 2: GUI（D&D / ジョブ一覧 / 設定パネル / 進捗 / キャンセル / 結果表示）` |
| base | `main` |
| merge commit | **`c67c36a`**（マージコミット方式。Phase 1 の PR #2 と同じ） |
| 取り込み | **21 コミット / 58 ファイル / +7,715 行 / -22 行** |
| `phase2` ブランチ | マージ後に削除（履歴は `main` に残る） |

### マージ前に確認したこと

| 確認 | 結果 |
|---|---|
| PR checks 4 件 | **すべて pass** |
| `mergeStateStatus` | `CLEAN`（コンフリクト無し） |
| 統合対象 `f89f6e5` でのローカル全テスト | **166 / 166 pass** |

`pull_request` と `push` の両トリガが走ったため、実質 8 ジョブが green だった。
...
**CI（run 31301218393、commit `c67c36a`）— 4 ジョブすべて success**
...
### Phase 2 の到達点（`docs/phases.md` §1 の「手元で実用できる」）

D&D でファイル / フォルダを追加し、出力先と拡張子を選び、進捗を見ながら変換し、
途中でキャンセルでき、失敗は理由付きで一覧に残る。テストは **93 → 166**。

`src/io` 層を新設し、`core → io → app` の依存方向を守った。
ADR-0008〜0011 を追加。機械検査に INV7 と ui1〜ui3 を追加した。

### クローズ時点で未達として残すもの（**「全項目達成」とは書かない**）

| 項目 | 状態 |
|---|---|
| 全機能がキーボードのみで操作可能（`spec-core.md` §7） | **未確認。** タブ順の連結と到達可能性は自動テスト済みだが、マウス無しの実操作は未確認 |
| Windows での実機起動 | **未確認。** Phase 0 から。CI では通っているが実機は別 |

**受け入れ基準は 6 項目中 5 項目が達成、1 項目（§7）が 6 / 7 の部分達成である。**
PR #3 の本文にも未達として明記し、テスト計画のチェックボックスも未チェックのまま残した。
**この 2 点が済むまで Phase 2 を「全項目達成」とは記録しない。**

### 品質ゲートの実行結果

上記「マージ後の `main` の検証」のとおり。
6 ゲート全数は直前の commit `1d41b44` で実行済み（dev / asan とも 166 / 166、警告 0、指摘 0）。
マージコミット `c67c36a` に対しては CI の 4 ジョブが全数を担っている。
...
1. **キーボードのみで開始・キャンセルできるか**（利用者にお願いする）。
   これが済むと `spec-core.md` §7 が 7 / 7 になり、Phase 2 の受け入れ基準が全項目達成になる。
2. **Windows での実機起動**（Phase 0 から未了）。
3. **`src/` 内の `QStringLiteral` を禁止する機械検査**の追加可否（前節の「提案」。未実装）。
4. Phase 3（拡張コーデック）は `docs/licenses.md` の更新が先（`docs/phases.md` §1）。
5. `.serena/` は未追跡のまま。
