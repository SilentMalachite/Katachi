...
## 追記（2026-08-17）: 公証は任意 / Windows 基準 5 は CI

利用者の承認により、Phase 4 着手時の受け入れ読みを次のように読み替える。
既存の論点 4・5 の「どこで実行するか / Windows は署名しない」は変えない。

### 決定 D15: macOS 公証は任意

**背景の受け入れ基準 1 にあった「署名・公証済み」を、「Developer ID 署名済み（公証は任意）」に
読み替える。** 公証の手順自体は `docs/release.md` §3.1 以降に残すが、必須経路ではない。

未公証の Developer ID 配布では `spctl` が `Unnotarized Developer ID` を返す。これは失敗ではなく
正しい状態である。受け手向けの開き方（右クリック → 開く、隔離属性の除去）を
`docs/release.md` §3.0 と README に書く。

### 決定 D14: Windows の受け入れ基準 5 は CI で足りる

着手時 D2 は「実機あり」と計画したが、**Windows 実機が無い**ため、基準 5 の Windows 側は
`release` プリセットの P9 と CI の `package` ジョブをもって達成とする。

**macOS 側の基準 5 は実機のまま残す**（Qt を持たない別アカウント）。CI の P8 は
macOS 実機の代替にはしない。
